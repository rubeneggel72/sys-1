  ...
  if (errors.length > 0) {
    ...
    try {
      ...
      try {
        ...
        //   <h4 style="font-style: italic;">You are now a cliente of our <strong>BOOK STORE</strong></h4>
      ...
      console.log("registro error: Internal error "+newUser)
...
router.post('/login',  passport.authenticate('local', {
  ...
  (req, res) => {res.render('0_home/home', { user: req.user});}
);
...
router.post('/order', checkAuthentication, async (req, res) => {
  ...
  const cliente = await stripe.clientes.create({
    email: stripeEmail,
    source: stripeToken
  });
  User.findById(req.user.id)
